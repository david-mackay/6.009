...
## BEGIN CODE WRITTEN BY DAVID MACKAY
...
class Image:
    ...
    def correlate(self, u):
        k = Image(int(len(u)**0.5), int(len(u)**0.5), u)
        distance_from_center = int((k.width-1)/2)
        result = Image.new(self.width, self.height)
        for x in range(result.width):
            for y in range(result.height):
                total = 0
                for kx in range(-distance_from_center, distance_from_center+1):
                    for ky in range(-distance_from_center, distance_from_center+1):
                        total += self.get_pixel(x+kx, y+ky)*k.get_pixel(distance_from_center+kx, distance_from_center+ky)
    ...
    def blurred(self, n):
        kernel = [(1/n)**2 for x in range(n**2)]
        y = self.correlate(kernel)
        list2 = []
        for i in y.pixels:
            if i>255:
                i= 255
        ...
        return Image(self.width, self.height, list2)
        
        

    def sharpened(self, n):
        kernel = [-(1/n)**2 for x in range(n**2)]
        kernel[len(kernel)//2]+=2
        y = self.correlate(kernel)
        list2 = []
        for i in y.pixels:
            if i>255:
    ...
    def edges(self):
        ...
        ox = self.correlate(k1)
        oy = self.correlate(k2)
        result = []
        for i in range(len(ox.pixels)):
            a = round((ox.pixels[i]**2 + oy.pixels[i]**2)**0.5)
            if a > 255:
                a=255
            if a<0:
                a = 0
        ...
        return Image(self.width, self.height, result)

    ## END CODE WRITTEN BY DAVID MACKAY
