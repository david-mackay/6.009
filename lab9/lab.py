...
class Game:
    ...
    def timestep(self, mouse=None):
        ...
        if self.status == 'ongoing':
            self.time += 1
            #self.move_remove_etc(self.formations, self.width, self.height)
            ...
            for i, animal in enumerate(self.formations): #HERE I MOVE ALL THE FORMATIONS. I USE A RENEWED REMOVAL LIST EACH TIMESTEP WHICH IS WHY I DO NOT MAKE NEW METHODS                                        # FOR MOVEMENT AND REMOVAL.
                ...
                if animal.type == 'animal':
                    animals.append(animal) #puts all the animals in a list
            ...
            for keeper in keepers:      
                if self.time%keeper.interval == keeper.mod: #keeper can throw food, check if keeper can see animal
                    ...
                    for animal in animals:
                        ...
                        if keeper.can_see(lines, origin, ray):
                            ...
                            new_food = Food(Constants.TEXTURES['food'], keeper.loc, (Constants.FOOD_WIDTH, Constants.FOOD_HEIGHT), keeper.throw_speed, keeper.aim_dir)
...
class Food(Formation):
    def __init__(self, texture, loc, size, vel, trajectory ):   
...
"""
each timestep: call formation.move()
set path to a list of all points
.move() should advance the loc by vel indices.
