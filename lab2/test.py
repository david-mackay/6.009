...
#        print("second test")
    ...
    def test_bacon_path(self):
        # Bacon path, tiny database, len 3.
        actor_id = 1640
        len_expected = [4724,2876,1640]

        first_result = lab.get_bacon_path(self.data, actor_id)
#        len_first_result = -1 if first_result is None else len(first_result)-1
        second_result = lab.get_bacon_path(self.data, actor_id)
#        len_second_result = -1 if second_result is None else len(second_result)-1

        self.assertTrue(valid_path(self.data, first_result))
        self.assertEqual(first_result, len_expected)
        self.assertEqual(first_result[0], 4724)
        self.assertEqual(first_result[-1], actor_id)
        
        self.assertTrue(valid_path(self.data, second_result))
        self.assertEqual(second_result, len_expected)
