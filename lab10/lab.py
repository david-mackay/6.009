...
#Rima is a cheap and powerful zookeeper, who gets frustrated very easily. Whenever he throws food and it misses, he get's extremely pissed and to keep him
#working we have to pay him more each time he misses. SO PLACE HIM WELL
################################################################################
        ...
        if self.status == 'ongoing':
            ...
            for form in removal_forms:
                ...
                if form.type == 'food':
                  ...
                  if form.thrown_by.keeper == 'Rima':
                      if form.animals_hit == 0:
                            if self.money - form.thrown_by.frustration <0:
                              self.money -= form.thrown_by.frustration #put you in debt and then leaves
                              form.thrown_by.stop_throwing()
                              form.thrown_by.quitting = True
                            else:
                                self.money -= form.thrown_by.frustration
                                form.thrown_by.frustration *= 2 #increase cost of miss
...
class Zookeeper(Formation): #record time of creation for food throwing interval
    ...
    def stop_throwing(self):
        self.oldinter = self.interval
        self.interval = float('inf')

    def resume(self):
        self.interval = self.oldinter
